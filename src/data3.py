...
LaserParams, Emiss, Wavelength = torch.FloatTensor, torch.FloatTensor, torch.FloatTensor


def get_data(use_cache: bool = False) -> Tuple[LaserParams, Emiss, Wavelength]:
    if all(
        [
            use_cache,
        ...
        laser_params, emissivity, wavelength = torch.load(Path("laser_params.pt")), torch.load(
            Path("emissivity.pt")), torch.load(Path("wavelength.pt"))
    else:
        ...
        for entry in tqdm(db.find()):
            ...
            wavelength_plot: List[float] = [
                ex["wavelength_micron"] for ex in entry["emissivity_spectrum"]
        ...
        laser_params, emissivity, wavelength = sklearn.utils.shuffle(laser_params, emissivity, wavelength)
    ...
    return laser_params, emissivity, wavelength
laser_params, emissivity, wavelength = get_data()