...
def emiss_error_graph(predicted_emissivity, real_emissivity, wavelen_num = 300):
    ...
    for i_run_index in range(50):
        ...
        for wavelen_i in range(wavelen_num):
            MSE_O_P += (real_emiss_list[wavelen_i] - current_list[wavelen_i]) ** 2
        RMSE_O_P = float(MSE_O_P / wavelen_num) ** (0.5)
        RMSE_total += RMSE_O_P/50

        RMSE_list.append(RMSE_O_P)

        if RMSE_O_P < best_RMSE:
            best_RMSE = RMSE_O_P
            best_run_index = i_run_index
        
        if RMSE_O_P > worst_RMSE:
            worst_RMSE = RMSE_O_P
    ...
    best_RMSO_Pred = predicted_emissivity[best_run_index][0:wavelen_num]

    best_RMSE_real = real_emissivity[best_run_index][0:wavelen_num]

    worst_RMSO_Pred = predicted_emissivity[worst_run_index][0:wavelen_num]

    worst_RMSE_real = real_emissivity[worst_run_index][0:wavelen_num]

    average_RMSO_Pred = predicted_emissivity[average_run_index][0:wavelen_num]

    MSE_O_P = 0
    for wavelen_i in range(wavelen_num):
        MSE_O_P += (real_emissivity[average_run_index][wavelen_i] - predicted_emissivity[average_run_index][wavelen_i]) ** 2
    RMSE_average = float(MSE_O_P / wavelen_num) ** (0.5)

    average_RMSE_real = real_emissivity[average_run_index][0:wavelen_num]

    return([best_RMSO_Pred, best_RMSE_real, worst_RMSO_Pred, worst_RMSE_real, average_RMSO_Pred, average_RMSE_real, wavelength, RMSE_total, RMSE_average])

def training_set_mean_vs_stdev():
    forward_train_data = torch.load("/data-new/alok/laser/data.pt")["interpolated_emissivity"]
...
pred_target: str
"""
def val_set_RMSE(pred_target = "/data-new/alok/laser/preds.pt"):
    ...
    for i in range(5):
        ...
        for p in range(400):
            ...
            RMSE_total += mean_squared_error(real_emiss_list, predicted_emiss_list, squared = False)
            print(RMSE_total)
...
def graph(residualsflag, predsvstrueflag, real_laser, real_emissivity, man_laser, man_emiss, predicted_laser, predicted_emissivity, wavelen_num = 800, index_str="0"):
    # importing the data
    ...
    for i in range(1):
        ...
        if residualsflag == True:
            ...
            # sort by watt, then speed, then spacing
                ...
                man = man_laser[p].T.cpu()
                watt1 = man.T[2:]
                watt2 = np.where(watt1 == 1)
                watt_man = (watt2[0] + 2)/10
                speed_man = man.T[:1].T.cpu()
                spacing_man = predicted.T[1:2].T.cpu()

                real_emiss_list = real_emissivity[p]
                predicted_emiss_list = predicted_emissivity[p]
                man_emiss_list = man_emiss[p]
                MSE_O_P = 0
                for wavelen_i in range(wavelen_num):
                    MSE_O_P += (real_emiss_list[wavelen_i]-predicted_emiss_list[wavelen_i])**2
                
                RMSE_O_P = (MSE_O_P/wavelen_num)**(0.5)
                if len(watt) == 1 and len(watt_pred) == 1:
                    ...
                    Laser_O_P_list.append(RMSE_expected_predicted)
                    Emiss_O_P_list.append(RMSE_O_P.detach())

                    
                MSE_O_M = 0
                for wavelen_i in range(wavelen_num):
                    MSE_O_M += (real_emiss_list[wavelen_i]-man_emiss_list[wavelen_i])**2
                
                RMSE_O_M = (MSE_O_M/wavelen_num)**(0.5)
                if len(watt) == 1 and len(watt_man) == 1:
                    watt_diff = (float(watt)-float(watt_man))**2
                    speed_diff = (float(speed)-float(speed_man))**2
                    space_diff = (float(spacing)-float(spacing_man))**2
                    RMSE_expected_manicted = ((watt_diff+speed_diff+space_diff)/3)**.5
                    Laser_O_M_list.append(RMSE_expected_manicted)
                    Emiss_O_M_list.append(RMSE_O_M.detach())

                    
                MSE_M_P = 0
                for wavelen_i in range(wavelen_num):
                    MSE_M_P += (real_emiss_list[wavelen_i]-predicted_emiss_list[wavelen_i])**2
                
                RMSE_M_P = (MSE_M_P/wavelen_num)**(0.5)
                if len(watt_pred) == 1 and len(watt_man) == 1:
                    watt_diff = (float(watt_pred)-float(watt_man))**2
                    speed_diff = (float(speed_pred)-float(speed_man))**2
                    space_diff = (float(spacing_pred)-float(spacing_man))**2
                    RMSE_expected_manicted = ((watt_diff+speed_diff+space_diff)/3)**.5
                    Laser_M_P_list.append(RMSE_expected_manicted)
                    Emiss_M_P_list.append(RMSE_M_P.detach())
    ...
    plt.scatter(
            ...
            alpha = 1,
            color = "green",
            label="New Man vs Pred Residuals",
        )
    ...
    plt.scatter(
            ...
            alpha = 1,
            color = "blue",
            label="New Man vs Orig Residuals",
        )
    ...
    # plt.annotate("r-squared = {:.3f}".format(r_value), (0, 1))
    plt.show()
    plt.savefig(f'{i}_final_emiss_laser_residual_graph.png')
...
def savO_Params(target_str, wavelen_num = 800):
    # importing the data
    wavelength = np.linspace(2.5, 12.5, num=800)
