...
def emiss_error_graph(predicted_emissivity, real_emissivity, save_str) -> None:
    #Emiss residuals
    ...
    for i_run_index in range(50):
        ...
        plt.plot(wavelength[0:820], old_emiss[0:820], c= 'blue', alpha = 0.1, linewidth = 2.0)
        MSE_E_P = 0
    ...
    # first_emiss = float(old_emiss[0])
    # new_emiss = np.concatenate((np.array([first_emiss for j in range(100)]), old_emiss))
    plt.scatter(wavelength[0:820], [0 for n in range(820)], s = [0.001 for n in range(820)], label = "Point density for reference")
    plt.plot(wavelength[0:820], old_emiss[0:820], c= 'blue', alpha = 0.1, linewidth = 2.0, label = f"Predicted Emiss, average RMSE {round(RMSE_total/50,5)}, MAPE {round(MAPE_total/50,5)}")

    new_emiss = real_emissivity[i_run_index]
    plt.plot(wavelength[0:820], new_emiss[0:820], c= 'black', alpha = 1, linewidth = 2.0, label = "Real Emissivity")
...
def graph(residualsflag, predsvstrueflag, index_str = "default", target_str = "0"):
    ...
    wavelength = torch.load(Path("wavelength.pt"))
    wavelength = np.flip(np.array(wavelength.cpu())[0])
    preds = torch.load(Path(f"{target_str}")) #this'll just be str() if I end up not needing it
    preds = preds[0]
