...
extension = torch.tensor(
    [
        extended_max - (i + 1) / granularity * (extended_max - extended_min)
...
if secondrun == True:
    for i_run_index in range(0, 400, 10):
        ...
        for arbitrary_vae in range(20):
            i = 0
            new_laser = torch.load(Path(f"preds_i_validation/preds_{i}_validation"))[0][
                "params"
            ][arbitrary_vae][i_run_index]

            real_emiss_list = torch.load(
                Path(f"preds_i_validation/preds_{i}_validation")
            )[0]["true_emiss"]
            # format the predicted params
            ...
            for i in range(20):
                ...
                old_emiss = np.flip(np.array(real_emissivity[rand_index].cpu()))
                first_emiss = float(old_emiss[0])
                new_emiss = np.concatenate(
                    (np.array([first_emiss for j in range(100)]), old_emiss)
                )
                new_FoMM = utils.planck_emiss_prod(
                    extended_wave, new_emiss, wavelen_cutoff, 1400
                )
                if new_FoMM > max_FoMM:
                    max_FoMM = new_FoMM
                    max_index = i
            ...
            plt.plot(
                ...
                c="limegreen",
                alpha=1,
                label=f"Best generated design, FoM {round(float(new_score),2)}",
                linewidth=3.0,
            )
